...
class DNAStorage(object):
    __slots__ = ('visGroups', 'DNAGroups', 'textures', 'fonts', 'fontFilenames', 'catalogCodes', 'nodes', 'hoodNodes', 'placeNodes', 
        'blockDoors', 'blockZones', 'blockNumbers', 'blockTitles', 'blockArticles', 'blockBuildingTypes', 'suitEdges', 'suitPoints', 'suitBlocks', 'suitBlockNumFloors', 'cogdoBlocks',)
    ...
    def getNumVisiblesInDNAVisGroup(self, index):
        return self.getDNAVisGroup(index).getNumVisibles()

    def getVisibleName(self, index, visibleIndex):
        return self.getDNAVisGroup(index).getVisible(visibleIndex)

    def getDNAVisGroup(self, index):
        return self.visGroups[index]

    def getDNAVisGroupName(self, index):
        return self.getDNAVisGroup(index).getName()
    ...
    def findNode(self, code):
        ...
        search = nodes[code][1]
        try:
           model = loader.loadModel(Filename(filename))
        except:
    ...
    def storeSuitBlockNumFloors(self, blockNumber, numFloors):
        self.suitBlockNumFloors[blockNumber] = numFloors

    def getNumFloors(self, blockNumber):
        return self.suitBlockNumFloors.get(blockNumber)
