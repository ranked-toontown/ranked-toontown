...
from otp.astron.AstronNetMessenger import AstronNetMessenger
...
class AstronInternalRepository(ConnectionRepository):
    ...
    def handleDatagram(self, di):
        ...
        elif msgType == MsgTypes.STATESERVER_OBJECT_GET_LOCATION_RESP:
    ...
    def handleGetObjectResp(self, di):
        ...
        if classId not in self.dclassesByNumber:
            self.notify.warning(
                'Received STATESERVER_OBJECT_GET_ALL_RESP for unknown dclass=%d! (Object %d)' % (classId, doId))
            return
